...
def importDiffMutScores(filename):
    scores = {}
    with open(filename, "r") as scoreFile:
        for line in scoreFile.readlines()[1:]:
           fields = line.split(" ")
           scores[fields[0]] = (float(fields[1]), float(fields[2]))
    return scores

def getDiffMutQvals(geneset):
  qvals = []
  for gene in geneset:
    qvals.append(diffMutScores[gene][1])
  return qvals
...
def measure(genes_collection1, genes_collection2):
  ...
  inside2 = 0
  #qvalues from diffMut for genes in genes_collection1
  qvals1 = getDiffMutQvals(genes_collection1)
  #qvalues from diffMut for genes in genes_collection2
  qvals2 = getDiffMutQvals(genes_collection2)
  ...
  for sampleID in sample_mutatedGenes:
    ...
    inside2 += num_ig2
  c = 0.5
  return c*float((2*out1 - inside1)*(1 - mean(qvals1)) - (2*out2 - inside2)*(1 - mean(qvals2)))
...
diffMutScores = importDiffMutScores("Output/" + sys.argv[1].split("/")[-1].split(".")[0] + "-DiffMut.txt")
